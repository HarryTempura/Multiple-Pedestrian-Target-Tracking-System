...
class Pedal(nn.Module):
    ...
    def forward(self, feature, centers, position, PatchMemory=None, vid=None, camid=None):
        ...
        for p in range(feature.size(0)):
            ...
            neg, index = dist_map[trick != position.unsqueeze(dim=1).expand_as(dist_map)].view(dist_map.size(0),
                                                                                               -1).sort(dim=1)
...
class Ipfl(nn.Module):
    ...
    def forward(self, feature, centers):
        ...
        image_label = torch.arange(feature.size(0) // self.num).repeat(self.num, 1).transpose(0, 1).contiguous().view(
            -1)
