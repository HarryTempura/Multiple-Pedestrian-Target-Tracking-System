...
class NaiveIdentitySampler(Sampler):
    ...
    def __init__(self, data_source: str, batch_size: int, num_instances: int, delete_rem: bool,
                 seed: Optional[int] = None, cfg=None):
        self.data_source = data_source
        self.batch_size = batch_size
        self.num_instances = num_instances
        self.num_pids_per_batch = batch_size // self.num_instances
        ...
        for pid in self.pids:
            ...
            if self.delete_rem:
                ...
                if len(idxs) < self.num_instances:  # if idxs is smaller than num_instance, choice redundantly
                    ...
                    idxs.extend(
                        np.random.choice(idxs, size=self.num_instances - len(idxs) % self.num_instances, replace=False))
...
class DomainSuffleSampler(Sampler):
    ...
    def __init__(self, data_source: str, batch_size: int, num_instances: int, delete_rem: bool,
        ...
        self.total_images = self.num_domains * (
                self.max_cnt_domains - (self.max_cnt_domains % self.batch_size) - self.batch_size)
            ...
            for pid in pids:
                ...
                if delete_rem:
                    ...
                    if len(idxs) < self.num_instances:  # if idxs is smaller than num_instance, choice redundantly
                        ...
                        idxs.extend(np.random.choice(idxs, size=self.num_instances - len(idxs) % self.num_instances,
                                                     replace=False))
        ...
        while len(avai_pids) >= self.num_pids_per_batch and not all(revive_idx):
            ...
            for i in range(self.num_domains):
                if len(local_avai_pids[i]) < self.num_pids_per_batch:
                    ...
                    batch_idxs_dict_new = _get_batch_idxs(self.pids, self.pid_index, self.num_instances,
                                                          self.delete_rem)
